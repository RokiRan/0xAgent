...
import { bootRegistry, postJson, getJson, pollMessages, } from './helpers.js';
// -----------------------------------------------------------------------------
...
function isHeldResponse(value) {
    return (hasField(value, 'held') &&
        value.held === true &&
        hasField(value, 'reason') &&
        typeof value.reason === 'string');
}
// 多处复用：把 held 响应里的 reason / unseen / token 抽出来，同时给出失败时的可读信息
function getHeldReason(body) {
    assert.ok(isHeldResponse(body), `expected held response, got ${JSON.stringify(body)}`);
    return body.reason;
}
function getUnseen(body) {
    assert.ok(isHeldResponse(body), `expected held response, got ${JSON.stringify(body)}`);
    return Array.isArray(body.unseen) ? body.unseen : [];
}
function getHoldToken(body) {
    assert.ok(isHeldResponse(body), `expected held response, got ${JSON.stringify(body)}`);
    assert.ok(typeof body.token === 'string' && body.token.length > 0, `expected non-empty token, got ${JSON.stringify(body.token)}`);
    return body.token;
}
...
function bcastMsg(from, channel, text, opts = {}) {
    const basePayload = { text };
    if (opts.human)
        basePayload.human = true;
    ...
    if (opts.holdToken)
        msg.holdToken = opts.holdToken;
    return msg;
}
function relayMsg(from, to, channel, payload) {
    return {
        id: `m-${Math.random().toString(36).slice(2)}`,
        type: 'event',
        from,
...
async function registerAgent(reg, agentId) {
...
async function listAgentIds(reg) {
    return getAgentsList(await getJson(reg.url, '/agents')).map((a) => a.agentId);
}
async function channelMembers(reg, channel) {
    return getMembersList(await getJson(reg.url, `/channels/members?channel=${encodeURIComponent(channel)}`));
}
async function listChannelNames(reg) {
...
test('relay: 发送者非渠道成员 → 403 "Sender not in channel"', async () => {
    ...
    try {
        ...
        const r = await postJson(reg.url, '/relay', relayMsg('outsider', 'member', 'priv', { x: 1 }));
...
test('relay: 目标非渠道成员 → 403 "Target not in channel"', async () => {
    ...
    try {
        ...
        const r = await postJson(reg.url, '/relay', relayMsg('sender', 'stranger', 'priv', { x: 1 }));
        assert.equal(r.status, 403);
...
test('relay: 渠道不存在 → 404', async () => {
    ...
    try {
        ...
        const r = await postJson(reg.url, '/relay', relayMsg('a', 'a', 'phantom', { x: 1 }));
        assert.equal(r.status, 404);
        assert.equal(getErrorMessage(r.body), 'Channel not found');
    }
    finally {
...
test('relay: 合法发送者与目标，目标 /poll 可取到消息', async () => {
    ...
    try {
        ...
        const r = await postJson(reg.url, '/relay', relayMsg('s', 't', 'default', { hello: 't' }));
        assert.equal(r.status, 200);
...
test('broadcast: 消息仅扇出到本渠道成员，跨渠道不可见', async () => {
    ...
    try {
        ...
        // d1 在 default 广播
        const r = await postJson(reg.url, '/broadcast', bcastMsg('d1', 'default', 'visible-to-default'));
        assert.equal(r.status, 200);
...
test('broadcast: 发送者非本渠道成员 → 403', async () => {
    ...
    try {
        ...
        const r = await postJson(reg.url, '/broadcast', bcastMsg('stranger', 'c', 'nope'));
        assert.equal(r.status, 403);
        assert.equal(getErrorMessage(r.body), 'Sender not in channel');
    }
    finally {
...
test('broadcast verbatim-dup: 同 (channel, from) 逐字重复 → 409 HELD verbatim', async () => {
    ...
    try {
        ...
        const first = await postJson(reg.url, '/broadcast', bcastMsg('v', 'default', 'same-text'));
        assert.equal(first.status, 200);
        const second = await postJson(reg.url, '/broadcast', bcastMsg('v', 'default', 'same-text'));
...
test('rate floor: 同 agent 60s 内 30 次广播后第 31 次 → 429', async () => {
    ...
    try {
        ...
        // 但 rate 计数器会逐次 +1
        for (let i = 0; i < 30; i++) {
            const r = await postJson(reg.url, '/broadcast', bcastMsg('r', 'rate', `text-${i}`));
            // 只校验是 200/409/429（HELD），不应有 4xx 其它错误码
            assert.ok(r.status === 200 || r.status === 409 || r.status === 429, `iteration ${i}: unexpected status ${r.status}`);
        }
        // 31st 必须在 rate 闸触发
        const r31 = await postJson(reg.url, '/broadcast', bcastMsg('r', 'rate', 'text-31'));
...
test('lapping（无人类）: 单一 speaker 第二次广播必 HELD', async () => {
    ...
    try {
        ...
        const first = await postJson(reg.url, '/broadcast', bcastMsg('l', 'lap', 'first'));
        ...
        const helperFirst = await postJson(reg.url, '/broadcast', bcastMsg('l-helper', 'lap', 'unread-for-l'));
        ...
        const second = await postJson(reg.url, '/broadcast', bcastMsg('l', 'lap', 'second'));
...
test('lapping: 人类 broadcast 后 agentMsgs/speakers 计数重置', async () => {
    ...
    try {
        ...
        const human = await postJson(reg.url, '/broadcast', bcastMsg('b', 'reset', 'human-msg', { human: true }));
        ...
        const after = await postJson(reg.url, '/broadcast', bcastMsg('a', 'reset', 'after-reset'));
...
test('freshness: agent 在有未读消息时广播 → 409 HELD 且内联 unseen', async () => {
    ...
    try {
        ...
        const seed = await postJson(reg.url, '/broadcast', bcastMsg('writer', 'f', 'unread-for-reader'));
        assert.equal(seed.status, 200);
        // reader 不 poll，直接 broadcast：必 HELD freshness 且 unseen 非空
        const held = await postJson(reg.url, '/broadcast', bcastMsg('reader', 'f', 'reader-msg'));
...
test('freshness: /poll 消费未读后 agent 可再广播', async () => {
    ...
    try {
        ...
        const blocked = await postJson(reg.url, '/broadcast', bcastMsg('r', 'p', 'reply'));
        ...
        const ok = await postJson(reg.url, '/broadcast', bcastMsg('r', 'p', 'reply-now'));
...
test('hold-token: 正确 token 一次性通过 freshness 闸', async () => {
    ...
    try {
        ...
        const held = await postJson(reg.url, '/broadcast', bcastMsg('r', 'h', 'first-try'));
        assert.equal(held.status, 409);
        const token = getHoldToken(held.body);
        // r 用 token 再 bcast → 应当 override 通过 freshness
        // 同样需要 cap 充足：r 是新 speaker，cap 从 1 升到 2
        const over = await postJson(reg.url, '/broadcast', bcastMsg('r', 'h', 'override-ok', { holdToken: token }));
...
test('hold-token: 同一 token 第二次使用 → 已被消费，闸仍触发', async () => {
    ...
    try {
        ...
        // 拿 token
        const held = await postJson(reg.url, '/broadcast', bcastMsg('r', 's', 'probe'));
        assert.equal(held.status, 409);
        const token = getHoldToken(held.body);
        // drain
        ...
        const firstUse = await postJson(reg.url, '/broadcast', bcastMsg('r', 's', 'use-1', { holdToken: token }));
        ...
        const secondUse = await postJson(reg.url, '/broadcast', bcastMsg('r', 's', 'use-2', { holdToken: token }));
...
test('hold-token: 错 seq 的 token（st.seq 已推进）→ override 拒绝', async () => {
    ...
    try {
        ...
        const held = await postJson(reg.url, '/broadcast', bcastMsg('r', 'q', 'probe'));
        assert.equal(held.status, 409);
        const tokenV1 = getHoldToken(held.body);
        // drain r 后让 r 用 tokenV1 通过（消耗，并让 r 进 speakers）
        await pollMessages(reg.url, 'r');
        const firstUse = await postJson(reg.url, '/broadcast', bcastMsg('r', 'q', 'use-v1', { holdToken: tokenV1 }));
        ...
        const held2 = await postJson(reg.url, '/broadcast', bcastMsg('r', 'q', 'probe-2'));
        ...
        const r2bcast = await postJson(reg.url, '/broadcast', bcastMsg('r2', 'q', 'r2-msg'));
        assert.equal(r2bcast.status, 200, `r2 bcast must succeed: ${JSON.stringify(r2bcast.body)}`);
        // drain r 后用 tokenV2：seq 已变，override 必失败
        await pollMessages(reg.url, 'r');
        const stale = await postJson(reg.url, '/broadcast', bcastMsg('r', 'q', 'stale-attempt', { holdToken: tokenV2 }));
        // 关键断言：200 表示 token 静默通过——这里不应 200
        if (stale.status === 200) {
            assert.fail(`stale token (wrong seq) must not silently pass; got 200: ${JSON.stringify(stale.body)}`);
        }
        ...
        if (stale.status === 409) {
            ...
            assert.ok(reason === 'freshness' || reason === 'lapping', `unexpected reason: ${reason}`);
        }
        else {
...
test('metrics: 每次 HELD 事件递增对应 held.<reason> 计数', async () => {
    ...
    try {
        ...
        // verbatim 至少 +1
        assert.ok((after.held.verbatim ?? 0) >= (before.held.verbatim ?? 0) + 1, `verbatim counter must increment: before=${before.held.verbatim} after=${after.held.verbatim}`);
        // freshness 至少 +1
        assert.ok((after.held.freshness ?? 0) >= (before.held.freshness ?? 0) + 1, `freshness counter must increment: before=${before.held.freshness} after=${after.held.freshness}`);
        // broadcasts 至少 +1（m2 那条成功）
        assert.ok(after.broadcasts >= before.broadcasts + 1, `broadcasts must increment: before=${before.broadcasts} after=${after.broadcasts}`);
    }
    finally {
        await reg.close();
    }
