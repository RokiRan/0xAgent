...
const ROOM = 'r-task';
function openTaskBoard(overrides = {}) {
    const db = makeDb();
...
test('create 校验：acceptance 空 → 抛错', () => {
    ...
    assert.throws(() => board.create(ROOM, '空验收', [], 'a1', 'approver', 'low'), /acceptance 不能为空/);
    board.close();
});
test('create 校验：owner==approver → 抛错（防自我确认）', () => {
    const { board } = openTaskBoard();
    assert.throws(() => board.create(ROOM, '自我确认', ['x'], 'same', 'same', 'low'), /approver 不得等于 owner/);
    board.close();
});
...
test('returnTask：review → in_progress 带 note，并触发 rework 派发', async () => {
    ...
    const { board, deps } = openTaskBoard({
        requestAgent: async (target, payload) => {
            ...
            // 额外记录所有派发的 action 序列
            const p = payload;
            if (p?.kind === 'task' && p.action)
                submitted.push(p.action);
            if (target !== 'a1')
                return {};
            // 第一次 assign 提交 evidence；之后的 rework 不再回 submit
            if (p && p.action === 'rework')
                return {};
            return { kind: 'task', action: 'submit', evidence: 'e1' };
        },
    });
    ...
    const reworkReq = deps.requests.find((r) => r.payload.action === 'rework');
    assert.ok(reworkReq, '应派发 rework');
    const pl = reworkReq.payload;
...
test('晋升门：单来源 promote 抛错；双来源 → 成功；pin 单来源 → 成功；promotedPrinciples 只含已晋升', () => {
    ...
    const principles = board.listPrinciples(ROOM);
    ...
    board.promotePrinciple(id);
    assert.equal(board.listPrinciples(ROOM)[0].promoted, 1);
    // 新增一条原则（单来源）通过人类 pin 晋升
    board.proposePrinciple(ROOM, '紧急变更需双人复核', t1.id);
    const pinCandidate = board.listPrinciples(ROOM)
